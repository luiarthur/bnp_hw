...
\frame{ %3
  ...
  \[\def\arraystretch{1.4}
    \begin{array}{rclcl}
      ...
      \sigma^2 &|& \m\theta_t^*, T^*, \m y, \sigma^2 &\sim& \text{IG}(a_{\sigma^2}+\frac{nT^*}{2},
        %b+\frac{\sum_{t=1}^{T^*}\m\theta^*'Hn^{-1}(\phi)\m\theta^*}{2})\\
      b_{\sigma^2}+\frac{\sum_{t=1}^{T^*}\m\theta^{*'} H_n^{-1}(\phi)\m\theta^*}{2})\\
      %p(\phi &|& \m\theta_t^*, T^*, \m y, \sigma^2) &\propto& \prod_{t=1}^{T^*} 
      %  ~\text{N}_n(\m\theta_t^* |\m 0_n,\sigma^2H_n(\phi)) ~)\\
      p(\phi &|& \m\theta_t^*, T^*, \m y, \sigma^2) &\propto& 
      [\phi]|H_n(\phi)|^{-T^*/2} \exp\left(-\frac{\sum_{t=1}^{T^*}\m\theta^{*'} H_n^{-1}(\phi)\m\theta^*}{2\sigma^2}\right)\\
...
\frame{ %4
  ...
  \[\def\arraystretch{1.4}
    \begin{array}{rclcl}
      \m\theta_t &|& \m{y_t}, \cdot &\sim& \text{N}_n(\tau^{-2}\m\Lambda(\mm), \m\Lambda)\\
                 && q0 &=& |\Lambda|^{1/2} \\
